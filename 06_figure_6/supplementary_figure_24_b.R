###################################################################
## COPYRIGHT UNIVERSITY OF CALIFORNIA, SAN FRANCISCO (UCSF) 2023 ##
## R. SHRESTHA, ET AL., D.A. QUIGLEY AND F.Y. FENG LABS, UCSF    ##
###################################################################

### LOAD LIBRARIES ---
library("stringr")
library("data.table")
library("fgsea")
library("ggplot2")
library("gridExtra")

### DEFINE PATH ---
dir.reproduce <- file.path("/data1/projects/WCDT_atac_2020/analysis/reproduce")
file.config_path <- file.path(dir.reproduce, "scripts/config/config_paths.R") 
source(file.config_path)

dir.tfpeaks2gene <- file.path(dir.reproduce_data, "tfpeaks2gene")
dir.footprints <- file.path(dir.reproduce_data, "tf_footprints/mcrpc_subtypes/data_footprints") 

### DEFINE FILES ---
file.tf2genes <- file.path(dir.tfpeaks2gene, "wcdt_tf2genes_r_0p4_pval_0p05_distTSS_500kb.tsv.gz")
file.tf_hits_rds <- file.path(dir.footprints, "tobias_mcrpc_subtypes_tf_hits.rds")
file.rds_genelist <- file.path(dir.reproduce_data, "genelist.rds")
file.rds_rnaseq <- file.path(dir.reproduce_data, "wcdt_rnaseq_atacseq_gene_expression.rds")
file.utility_functions <- file.path(dir.reproduce_scripts, "00_prepare_data/00_01_utility_functions.R")

### LOAD FUNCTIONS ---
source(file.utility_functions)

### MOTIFS ---
motif_prefix_znf <- "MA0528.1_ZNF263"
motif_prefix_myc <- "MA0147.3_MYC"

############################################################################################################################################
########## PREPARE RNA-SEQ EXPRESSION DATA -----------------------------------------------------------------------------------
### LOAD RNA-SEQ GENE EXPRESSION OBJECT: TPM ---
cat(format(Sys.time(), "%b %d %X"), "LOADAING R DATA-OBJECT: RNA-seq EXPRESSION ... ", "\n", sep=" ")
    list.rnaseq <- readRDS(file=file.rds_rnaseq)

    # ANNOTATION ---
    gannot <- list.rnaseq$gene_annotation
    gannot <- subset(gannot, gannot$GeneType == "protein_coding")
    gannot <- gannot[-which(stringr::str_detect(gannot$EnsemblID, "PAR_Y")),]


############################################################################################################################################
### LOAD GENELIST ---
list.rds_genelist <- readRDS(file=file.rds_genelist)
list.genelist_kegg <- list.rds_genelist$msigdb_pathways$kegg
list.genelist_reactome <- list.rds_genelist$msigdb_pathways$reactome
list.genelist_wikipathways <- list.rds_genelist$msigdb_pathways$wikipathways
list.genelist_gobp <- list.rds_genelist$msigdb_pathways$gobp
list.genelist_hallmark <- list.rds_genelist$msigdb_pathways$hallmark

### LOAD TF2GENES DATA ---
dat_tf2genes <- data.table::fread(file=file.tf2genes, sep="\t", header=TRUE, nThread=50, data.table=FALSE, verbose=FALSE)

### GET TARGET GENES ---
genes_znf <- unique(dat_tf2genes$Gene[which(dat_tf2genes$motif_prefix == motif_prefix_znf)])
genes_myc <- unique(dat_tf2genes$Gene[which(dat_tf2genes$motif_prefix == motif_prefix_myc)])
genes_znf_myc <- intersect(genes_znf, genes_myc)


### RUN FGSEA ORA ---
set.seed(12345)
dat_ora_hallmark <- fgsea::fora(pathways=list.genelist_hallmark, genes=genes_znf_myc, universe=gannot$Gene, minSize=5, maxSize=500)
dat_ora_hallmark_pass <- subset(dat_ora_hallmark, dat_ora_hallmark$padj < 0.01)


############################################### PLOT ENRICHMENT ############################################################################
### FUNCTION: getPathwaysData() ---
getPathwaysData <- function(dat){
    # EXTRACT DATA ---
    if(nrow(dat) >= 20){
        dat <- dat[1:20,]
    }

    # ADD PVALUES ---
    pval <- min(setdiff(dat$pval, 0))
    index0 <- which(dat$pval == 0)

    if(length(index0) != 0){
        dat$pval[which(dat$pval == 0)] <- pval
    }

    # ADD VALUES ---
    dat$nlogp <- -log10(dat$padj)
    dat <- dat[order(dat$nlogp, decreasing=FALSE),]
    dat$pathway <- factor(dat$pathway, levels=dat$pathway)

    return(dat)
}

### FUNCTION: plotBar() ----
plotBar <- function(df){
    # PLOT ---
    p <- ggplot(df, aes(x=nlogp, y=pathway)) +
            geom_bar(stat="identity", fill="#006094") +
            coord_cartesian(xlim=c(0,24)) +
            scale_x_continuous(breaks=seq(0,24,by=4)) +
            theme(
                axis.text.x = element_text(size = 5, color="#000000"),
                axis.text.y = element_text(size = 5, color="#000000"),
                axis.title = element_text(size = 5, color="#000000"),
                plot.title = element_text(size = 5, color="#000000", hjust=0.5),
                panel.grid.major = element_blank(),
                panel.grid.minor = element_blank(),
                axis.ticks = element_line(size=0.2, color="#000000"), 
                strip.text = element_text(size=5, color="#000000"),
                strip.background = element_rect(fill="#FFFFFF", color="#FFFFFF"),
                panel.background = element_rect(fill="#FFFFFF", color="#000000"),
                legend.text = element_text(size = 5, color="#000000"),
                legend.title = element_blank(),
                legend.key.size = unit(0.3, "cm"),
                legend.position = "none") +
            xlab("-log10(qvalue)") +
            ylab("") + 
            ggtitle("") 
    
    return(p)
}



### LOAD ENRICHMENT DATA ---
dat_ora_parsed <- getPathwaysData(dat=dat_ora_hallmark_pass)

### GET PLOT ---
p_bar <- plotBar(df=dat_ora_parsed)

### PLOT ---
file.plot <- file.path(dir.reproduce_fig, "supplementary_figure_24_b.pdf")
pdf(file.plot, width=2.5, height=2)
    grid.arrange(p_bar, nrow=1, ncol=1)
dev.off()

