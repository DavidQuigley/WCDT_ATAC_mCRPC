...
metadata <- subset(metadata, metadata$mCRPC_Subtype == "ARL_NE-")
...
list.footprints_bound <- list.footprints_bound[metadata$Sample_Name]
...
list.footprints_unbound <- list.footprints_unbound[metadata$Sample_Name]
...
### GET VOLCANO PLOT ---
p1 <- getVolcanoPlot(file=NULL, df=df, logfc.threshold=1, pvalue.threshold=0.05, filterby.pvalueadj=FALSE, fc.upperbound=6, nlogp.upperbound=6, label_text=FALSE)
p2 <- getVolcanoPlot(file=NULL, df=df, logfc.threshold=1, pvalue.threshold=0.05, filterby.pvalueadj=FALSE, fc.upperbound=6, nlogp.upperbound=6, label_text=TRUE)


### PLOT ---
file.plot <- file.path(dir.reproduce_fig, "supplementary_figure_22_d.pdf")
