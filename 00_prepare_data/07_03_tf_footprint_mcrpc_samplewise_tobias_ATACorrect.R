...
### GET FILES ---
files.bam <- sapply(sample_names, function(x) { paste(dir.footprint, "/", sprintf("%s/%s", x, x), ".bam", sep="") })
...
for(sample_name in sample_names){
    # GET SOURCE FILE ---
    file.bam <- files.bam[sample_name]
    file.bed_peaks <- files.bed[sample_name]
    dir.output <- file.path(dir.footprints, sample_name)

    # GET COMMANDS ---
    list.cmd_sge <- get.SGEcmd(jobid=paste("ta", sample_name, sep="_"), dir.logs)
