...
file.motif_db <- file.path(dir.reproduce, "reference/JASPAR2018_AR_FOXA1_HOXB13.meme")
...
getBindetect <- function(dir.data, dir.output, file.motif_db, file.ref_genome, file.bed_peaks, file.header_peaks, bound_pvalue, analysis, group_id_1, group_id_2){
    # DEFINE FILES ---
    dir.output_group <- file.path(dir.output, paste(analysis, paste(group_id_1, group_id_2, sep="_"), sep="_")  )
    ...
    line10 <- paste("  --bound-pvalue", bound_pvalue, "\\", sep=" ")
    line11 <- paste("  --outdir ${DIR_OUTPUT}", "\\", sep=" ")
    line12 <- paste("  --skip-excel", "\\", sep=" ")
    line14 <- paste("  --cores 60", "\\", sep=" ")
    line15 <- paste("  --verbosity 5", sep=" ")

    # COMPILE COMMAND ---
    list.cmd <- list(ln_1, ln_2, ln_3, ln_4, ln_5, ln_6, ln_7, ln_8, ln_9, ln_10, 
                    line1, line2, line3, line4, line5, line6, line7, line8, line9, line10, line11, line12, line14, line15)
...
for(i in 1:length(pvals)){
    for(k in 1:nrow(dm_grp)){
        ...
        list.cmd_tobias <- getBindetect(dir.data=dir.footprints, dir.output=dir.footprints, file.motif_db, file.ref_genome, file.bed_peaks, file.header_peaks, bound_pvalue=pvals[i], analysis=names(pvals)[i], group_id_1=group_id_1, group_id_2=group_id_2)

        # MERGE COMMANDS ---
        cmd <- c(unlist(list.cmd_sge), unlist(list.cmd_tobias))

        # WRITE QSUB SCRIPT --
        file.qsub <- file.path(dir.scripts, paste("qsub_tobias_bindetect_", names(pvals)[i], "_", id, ".sh", sep=""))
        write.table(cmd, file.qsub, row.names=FALSE, col.names=FALSE, quote=FALSE)

        list.sh[[ctr]] <- paste("qsub", file.qsub, sep=" ")
        ctr <- ctr + 1

        cat("PROCESSED:", names(pvals)[i], id, "\n", sep=" ")
    }
}
